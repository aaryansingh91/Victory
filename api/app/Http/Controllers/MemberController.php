...
class MemberController extends Controller {
    ...
    protected $base_url;
    protected $profile_img_size_array;
    ...
    public function __construct() {
        ...
        $this->profile_img_size_array = array(100 => 100 , 200=>200);
...
//     }
    ...
    public function getAllGame($game_id) {				
        ...
        $data['all_game'] = DB::table('game as g')
                ->select('*', \DB::raw('(select count(*) from matches as m where m.game_id = g.game_id and m.match_status = "1") as total_upcoming_match'),\DB::raw('(select count(*) from ludo_challenge as l where l.game_id = g.game_id and l.accept_status = "0" and l.challenge_status = "1" and l.member_id != "'. $member_id .'" and l.accepted_member_id != "'. $member_id .'") as total_upcoming_challenge'), \DB::raw('(CASE 
                        ...
                        ELSE CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/game_image/thumb/500x500_", g.game_image) 
                        END) AS game_image'), \DB::raw('(CASE 
                        WHEN g.game_logo = "" THEN "" 
                        ELSE CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/game_logo_image/thumb/200x200_", g.game_logo) 
                        END) AS game_logo'))
                ->where("status", '1')
                
                ->orderBy('game_id', 'ASC')
                ->where('home_game_id', $game_id)
                ->get();
    ...
    public function getHomeGame() {				
        ...
        $data['all_game'] = DB::table('home_game')
            ->selectRaw(
                '*, 
                CASE 
                    WHEN home_game.game_image = "" THEN "" 
                    ELSE CONCAT(?, "/", ?, "/game_image/thumb/500x500_", home_game.game_image) 
                END AS game_image',
                [$this->base_url, $this->system_config['admin_photo']]
            )
            ->where("status", '1')
            ->orderBy('game_id', 'ASC')
            ->get();
    ...
    public function getAllOngoingMatch($game_id, $member_id) {
        ...
        $data['all_ongoing_match'] = DB::table('matches')
                ...
                ->select('m_id', 'match_name', 'match_url', 'matches.result_type', 'matches.room_description', 'match_time', 'matches.win_prize', 'prize_description', 'per_kill', 'entry_fee', 'type', 'MAP', 'match_type', 'match_desc','match_private_desc', 'no_of_player', 'number_of_position', \DB::raw('(CASE 
    ...
    public function getAllGameResult($game_id, $member_id) {
        ...
        $data['all_game_result'] = DB::table('matches as m')
                ...
                ->where("game_id", $game_id)
                ->select('m_id', 'match_name', 'match_url', 'm.result_type', 'm.room_description', DB::raw("STR_TO_DATE(match_time, '%d/%m/%Y %h:%i %p') as m_time"), 'match_time', 'm.win_prize', 'prize_description', 'per_kill', 'entry_fee', 'type', 'MAP', 'match_type', 'match_desc','match_private_desc', 'no_of_player', 'number_of_position', \DB::raw('(CASE 
    ...
    public function getAllPlayMatch($game_id, $member_id) {
        ...
        $data['allplay_match'] = DB::table('matches as m')
                ->leftJoin('image as i', 'i.image_id', '=', 'm.image_id')
                ->select('pin_match', 'm_id', 'match_name', 'match_url', 'm.result_type', 'm.room_description', DB::raw("STR_TO_DATE(match_time, '%d/%m/%Y %h:%i %p') as m_time"), 'match_time', 'm.win_prize', 'prize_description', 'per_kill', 'entry_fee', 'type', 'MAP', 'match_type', 'match_desc','match_private_desc', 'no_of_player', 'number_of_position', \DB::raw('(CASE 
                        WHEN m.image_id != 0 THEN CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/select_image/thumb/1000x500_", i.image_name) 
                        WHEN m.match_banner != "" THEN CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/match_banner_image/thumb/1000x500_", m.match_banner) 
                        ELSE ""
                        END) AS match_banner'), 'match_sponsor')
    ...
    public function getMyMatches($member_id) {
        ...
        $data['my_match'] = DB::table('match_join_member as mj')
                ...
                ->select('g.game_name', 'm_id', 'match_name', 'match_url', 'm.result_type', 'm.room_description', DB::raw("STR_TO_DATE(match_time, '%d/%m/%Y %h:%i %p') as m_time"), 'match_time', 'm.win_prize', 'prize_description', 'per_kill', 'entry_fee', 'type', 'MAP', 'match_type', 'match_desc','match_private_desc', 'no_of_player', 'number_of_position', \DB::raw('(CASE 
                        WHEN m.image_id != 0 THEN CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/select_image/thumb/1000x500_", i.image_name) 
                        WHEN m.match_banner != "" THEN CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/match_banner_image/thumb/1000x500_", m.match_banner) 
                        ELSE ""
    ...
    // echo $this->system_config['admin_photo'] ;exit;
        $member_id = Auth::user()->member_id;
        $data['member'] = DB::table('member')
                ->where("member_id", $member_id)
                ->select('password', 'first_name','last_name', 'email_id', 'user_name', 'wallet_balance', 'join_money', 'pubg_id', 'member_status',DB::raw('(CASE 
                WHEN profile_image = "" THEN "hfg"
                ELSE CONCAT ("' . $this->base_url . '/' . $this->system_config['admin_photo'] . '/profile_image/thumb/200x200_", profile_image) 
    ...
    $abc = DB::table('match_join_member as mj')
        ->where("match_id", $match_id)
        ->leftJoin('member as m', 'mj.member_id', '=', 'm.member_id')
        ->select('user_name', 'mj.pubg_id', 'killed', 'total_win')
        ->where('total_win', '>', 0) // Exclude records with total_win = 0
        ->orderBy('win_prize', 'DESC')
        ->orderBy('total_win', 'DESC')
        ->get()
        ->toArray();
        ...
        $data['match_winner'] = array_slice($abc, 0, $limit);
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    public function sendOTP(Request $request) {
        ...
        if ($member->count() > 0) {
            if (strtolower($member[0]->email_id) == strtolower($request->input('email_mobile'))) {
                ...
                $emailFrom = $this->system_config['smtp_user'];
                $emailFromName = $this->system_config['company_name'];
                $emailTo = $request->input('email_mobile');

                if (!filter_var($emailTo, FILTER_VALIDATE_EMAIL)) {
                    exit(json_encode(['status' => false, 'message' => 'Invalid email address']));
                }
                ...
                try {
                    ...
                    $mail->Body = "<html>
                                    ...
                                    <body>
                                    <p>Your verification OTP is: " . htmlspecialchars($otp) . "</p>                            
                                    </body>
                                    </html>";
                    ...
                    $response = [
                        'status' => true,
                        'title' => 'Success!',
                        'message' => trans('message.text_succ_send_mail'),
                        'member_id' => $member[0]->member_id ?? null,
                        'otp' => $otp,
                    ];
                } catch (Exception $e) {
...
//                }
            ...
            if ($member) {
                ...
                if ($member->wallet_balance >= $request->input('amount')) {
                    ...
                    try {
                        ...
                        $mail->addAddress($emailTo);
                        $mail->isHTML(true);
                        $mail->Subject = "Withdrawal request from " . htmlspecialchars($member->email_id);
                        $mail->Body = "
                            <html>
                                <head>
                                ...
                                <body>
                                    <p>Withdrawal requested by <strong>" . htmlspecialchars($member->email_id) . "</strong> of INR <strong>" . htmlspecialchars($request->input('amount')) . "</strong>.</p>                            
                                </body>
                            </html>";
                        ...
                        Log::error('Error sending withdrawal email: ' . $mail->ErrorInfo, [ 
                            'email' => $emailTo,
                            'exception' => $e->getMessage(),
                        ]);
    ...
    public function oneSignalApp() {
        $array = ["one_signal_rest_api_key" => $this->system_config['rest_api_key'],"one_signal_app_id" => $this->system_config['app_id'], "one_signal_notification" => $this->system_config['one_signal_notification']];
        echo json_encode($array,JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    public function version($versionfor) {
        ...
        if ($versionfor == 'android') {
            ...
            if (!$currency) {
                ...
                echo json_encode(["error" => "Currency configuration not found"], JSON_UNESCAPED_UNICODE);
                exit;
            }
            ...
            $array = [
                ...
                "firebase_key" => $this->system_config['app_id'],
                "currency_symbol" => $currency->currency_symbol,
                "banner_ads_show" => $this->system_config['banner_ads_show'],
                "fb_login" => $this->system_config['fb_login'],
                "google_login" => $this->system_config['google_login'],
                "firebase_otp" => $this->system_config['firebase_otp'],
                "version" => $app_upload ? $app_upload->app_version : '1',
                "force_update" => $app_upload ? $app_upload->force_update : "No",
                "force_logged_out" => $app_upload ? $app_upload->force_logged_out : "No",
                "url" => $this->base_url . '/' . $this->system_config['admin_photo'] . '/apk/' . ($app_upload->app_upload ?? ''),
                "description" => $app_upload->app_description ?? ''
            ];
    ...
    public function getPayment() {
        ...
        foreach ($payments as $payment) {
            ...
            } else if ($payment->payment_name == 'UPITranzact') {
                $data['payment'][$i]['mid'] = $payment->mkey;
                $data['payment'][$i]['mkey'] = $payment->wname;
                $data['payment'][$i]['e_url'] = $payment->payment_description;
            } else if ($payment->payment_name == 'UPITranzact Business') {
                $data['payment'][$i]['mid'] = $payment->mid;
                $data['payment'][$i]['pkey'] = $payment->mkey;
                $data['payment'][$i]['mkey'] = $payment->wname;
            }
...
//        $row = DB::table('pg_detail')->where('payment_name', $request->input('payment_name'))->first();
        if ($row->payment_name == 'Instamojo') {
            ...
            $create_order_payload = array(
                'receipt' => (string)$id,
                'amount' => $this->convertAmount($amount, $row->currency_code),
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_POSTFIELDS =>json_encode($create_order_payload),
            ...
            $array['title'] = 'Success!';
            $array['message'] = 'success';
            $array['order_id'] = $response_decode->id;
            $array['receipt'] = $id;
            ...
            $cashfreepayload = array(
                'order_amount' => $request->input('TXN_AMOUNT'),
                'order_currency' => "INR",
                'customer_details' => array(
                    'customer_id' => $request->input('CUST_ID'),
                    'customer_name' => $member_data->user_name,
                    'customer_email' => $member_data->email_id,
                    'customer_phone' => $member_data->mobile_no
                ),
                'order_meta' => array(
                    'return_url' => $this->base_url.'/cashfree_response'
                )
            );
            
            if ($row->payment_status == 'Production')
                $transactionURL = "https://api.cashfree.com/pg/orders"; // for production
            else
                $transactionURL = "https://sandbox.cashfree.com/pg/orders";;
            ...
            curl_setopt_array($curl, array(
              ...
              CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
              CURLOPT_CUSTOMREQUEST => 'POST',
              CURLOPT_POSTFIELDS => json_encode($cashfreepayload),
              CURLOPT_HTTPHEADER => array(
            ...
            $response = json_decode($response, true);
             ...
             $update_deposit = array(
                'bank_transection_no' => $response['order_id']
            );
        ...
        } elseif ($row->payment_name == 'UPITranzact') {
            ...
            echo json_encode($array, JSON_UNESCAPED_SLASHES);
        } elseif ($row->payment_name == 'UPITranzact Business') {
            $bank_transection_no = str_replace(".", "", microtime(true)) . rand(000, 999);
            $deposit_data = [
                'member_id' => $request->input('CUST_ID'),
                'deposit_amount' => $request->input('TXN_AMOUNT'),
            ...
            //             'member_id' => $request->input('CUST_ID'),
            //             'pubg_id' => "",
            //             'deposit' => $request->input('TXN_AMOUNT'),
            //             'withdraw' => 0,
            //             'join_money' => "",
            //             'win_money' => "",
    ...
    public function convertAmount($amount, $currency) {
        ...
        if (in_array($currency, $threeDecimalCurrencies)) {
            // Convert to smallest sub-unit for three decimal currencies
            $convertedAmount = floor($amount * 1000);
        } elseif (in_array($currency, $zeroDecimalCurrencies)) {
            // Convert to smallest sub-unit for zero decimal currencies
            $convertedAmount = round($amount);
        } else {
            // Convert to smallest sub-unit for two decimal currencies (default)
            $convertedAmount = round($amount * 100);
        }
    ...
    public function googlePayResponse(Request $request) {
        ...
        $pg_detail = DB::table('pg_detail')->where('payment_name', 'UPITranzact')
                        ->leftJoin('currency as c', 'c.currency_id', '=', 'pg_detail.currency')
                        ->select('pg_detail.*', 'c.currency_name', 'c.currency_code', 'c.currency_symbol')->first();
        $order = DB::table('deposit')
        ...
        if ($request->input('status') == "true" && $order->deposit_status == 0) {
            $deposit_data = [
                'deposit_status' => '1',];
            $res = DB::table('deposit')->where('bank_transection_no', $request->input('transaction_id'))->where('deposit_id', $request->input('order_id'))->update($deposit_data);
            $row = DB::table('member')
                    ->where('member_id', $request->input('member_id'))
                    ->first();
            $join_money = $row->join_money + ($request->input('amount'));
    ...
    public function upitranzactResponse(Request $request) {
    $validator = Validator::make($request->all(), [
        'member_id' => 'required',
        'amount' => 'required|numeric|min:' . $this->system_config['min_addmoney'],
        'transaction_id' => 'required',
    ...
    $pg_detail = DB::table('pg_detail')
        ->where('payment_name', 'UPITranzact Business')
        ->leftJoin('currency as c', 'c.currency_id', '=', 'pg_detail.currency')
        ->select('pg_detail.*', 'c.currency_name', 'c.currency_code', 'c.currency_symbol')
        ->first();
    ...
    if ($order->deposit_status != 0) {
        $array['status'] = false;
        $array['title'] = 'Error!';
        $array['message'] = 'Order already processed';
        echo json_encode($array, JSON_UNESCAPED_UNICODE);
        exit;
    }
    
    // CRITICAL FIX: Verify with NeoUPI API
    try {
        $curl = curl_init();
        
        $payload = json_encode([
            'public_key' => $pg_detail->mid,
            'secret_key' => $pg_detail->mkey,
            'order_id' => $order->bank_transection_no
        ]);
        
        curl_setopt_array($curl, [
            CURLOPT_URL => 'https://neoupi.com/apis/v1/order-status',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
        ...
        if ($httpCode != 200 || !$response) {
            DB::table('deposit')->where('deposit_id', $order->deposit_id)->update(['deposit_status' => '2']);
            $array['status'] = false;
            $array['title'] = 'Error!';
            $array['message'] = trans('message.text_err_balance_not_add');
            echo json_encode($array, JSON_UNESCAPED_UNICODE);
        ...
        if (!isset($result['order_details']['STATUS']) || 
            strtoupper($result['order_details']['STATUS']) !== 'SUCCESS') {
            DB::table('deposit')->where('deposit_id', $order->deposit_id)->update(['deposit_status' => '2']);
            $array['status'] = false;
            $array['title'] = 'Error!';
            $array['message'] = trans('message.text_err_balance_not_add');
        ...
        $verifiedAmount = floatval($result['order_details']['AMOUNT']);
        $orderAmount = floatval($order->deposit_amount);
        
        if ($verifiedAmount != $orderAmount) {
            DB::table('deposit')->where('deposit_id', $order->deposit_id)->update(['deposit_status' => '2']);
            $array['status'] = false;
            $array['title'] = 'Error!';
            $array['message'] = 'Payment amount mismatch';
            echo json_encode($array, JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        // Verify order ID matches
        if ($result['order_details']['ORDERID'] != $order->bank_transection_no) {
            DB::table('deposit')->where('deposit_id', $order->deposit_id)->update(['deposit_status' => '2']);
            $array['status'] = false;
            $array['title'] = 'Error!';
            $array['message'] = 'Order ID mismatch';
            echo json_encode($array, JSON_UNESCAPED_UNICODE);
            exit;
        }
    ...
    if ($request->input('status') == "true" && $order->deposit_status == 0) {
        $deposit_data = ['deposit_status' => '1'];
        $res = DB::table('deposit')
            ->where('bank_transection_no', $request->input('transaction_id'))
            ->where('deposit_id', $request->input('order_id'))
            ->update($deposit_data);
    ...
    public function send_onesignal_noti_old($heading_msg,$content_msg,$player_id,$member_id,$game_id,$multi = false){
    ...
    public function send_onesignal_noti($heading_msg,$content_msg,$player_id,$member_id,$game_id,$multi = false){
        ...
        if($this->system_config['one_signal_notification']){
            ...
            include ('./kreit_firebase/vendor/autoload.php');
            $jsonFilePath = base_path() . '/storage/app/firebase_config.json';
            $firebase_database_url = $this->system_config['firebase_realtime_db_url'];
            ...
            $notification = \Kreait\Firebase\Messaging\Notification::create($heading_msg, $content_msg);
            ...
            $sendReport = $messaging->sendMulticast($message, $player_id);
