...
class Page extends CI_Controller {
    ...
    public function index($id = 'home') {
        ...
        $data['content'] = $this->functions->getPage($id);
        if ($id == 'home') {
            $this->db->select('*');
...
//        $phone = $this->input->post('phone');
        ...
        $this->form_validation->set_rules('fname', 'lang:text_full_name', 'required', array('required' => $this->lang->line('err_fname_req')));
    ...
    function delete_FBUser() {
        $this->db->where('fb_id', $this->input->get('id'));
        $qr = $this->db->get('member');
        echo $this->db->last_query();
        exit;
        print_r($qr->row_array());
        exit;
    }
