...
class Matches extends CI_Controller {
    ...
    function index() {
        $data['match'] = true;
        // Check if user has add_match permission to show the button
        if($this->functions->check_permission('add_match')) {
    ...
    function setDatatableMatch() {
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            // Check permissions for cancel and deactivate options
            $can_cancel = $this->functions->check_permission('cancel_match');
            $can_deactivate = $this->functions->check_permission('deactivate_match');
            ...
            if ($row['match_status'] == '0') {
                $nestedData[] = '<select onChange="javascript: changePublishStatus(document.frmmatchlist,' . $row['m_id'] . ',this.value);">'
                        ...
                        . ($can_cancel ? '<option value="4">Cancel</option>' : '') . '</select>';                      
            } elseif ($row['match_status'] == '1') {
                $nestedData[] = '<select onChange="javascript: changePublishStatus(document.frmmatchlist,' . $row['m_id'] . ',this.value);">'
                        . ($can_deactivate ? '<option value="0" >Deactive</option>' : '')
    ...
    function member_join_match() {
        ...
        if($data['match']['result_type'] == 1 || $data['match']['result_type'] == '1') {
            $this->load->view($this->path_to_view_admin . 'member_join_match_fixed', $data);
        } else {
    ...
    function insert() {
        // Check if user has add_match permission
        if(!$this->functions->check_permission('add_match')) {
            $this->session->set_flashdata('error', 'You do not have permission to add matches');
            redirect($this->path_to_view_admin . 'matches');
        }
    ...
    function edit() {
        // Check if user has only room details permission
        $only_room_details = $this->functions->check_permission('only_room_details');
        
        if(!$this->functions->check_permission('matches_edit') && !$only_room_details) {
            $this->session->set_flashdata('error', $this->lang->line('text_err_edit_match'));
            redirect($this->path_to_view_admin . 'matches');
        }

        $data['match_addedit'] = true;
        $data['only_room_details'] = $only_room_details; // Pass to view
        $m_id = $this->uri->segment('4');
        ...
        if ($this->input->post('id_submit') == $this->lang->line('text_btn_submit')) {
            ...
            // Only allow full edit if user doesn't have only_room_details permission
            if($only_room_details) {
                $this->session->set_flashdata('error', 'You only have permission to edit room details');
                redirect($this->path_to_view_admin . 'matches/edit/' . $m_id);
            }
