...
//            parent::CI_Controller();         
    ...
    function index() {
        ...
        $data['title'] = $this->lang->line('text_appsetting');

        if ($this->input->post('submit') == $this->lang->line('text_btn_submit')) {
            if ($this->system->demo_user == 1) {
                $this->session->set_flashdata('error', $this->lang->line('text_err_upload_app'));
                ...
                $this->form_validation->set_rules('app_version', 'lang:text_version_code', 'required', array('required' => $this->lang->line('err_app_version_req')));
                $this->form_validation->set_rules('force_update', 'lang:text_force_update', 'required', array('required' => $this->lang->line('err_force_update_req')));
                $this->form_validation->set_rules('app_description', 'lang:text_app_desc', 'required', array('required' => $this->lang->line('err_app_desc_req')));

                if ($this->form_validation->run() == FALSE) {
                    $this->load->view($this->path_to_view_admin . 'appsetting_manage', $data);
                } else {
                    if ($result = $this->appsetting->insert()) {
                        $this->session->set_flashdata('notification', $this->lang->line('text_succ_upload_app'));
                        redirect($this->path_to_view_admin . 'appsetting/');
                    }
                }
            }
        } else if ($this->input->post('reset_leaderboard_submit')) {
            
            $this->db->where('web_config_name', 'reset_leaderboard');
            $this->db->set('web_config_value', date('Y-m-d H:i:s'));
            $this->db->update('web_config');
            $this->session->set_flashdata('notification', 'Leaderboard reset successfully');
            redirect($this->path_to_view_admin . 'appsetting/');
            
        } else if ($this->input->post('show_all_leaderboard_submit')) {
            
            $this->db->where('web_config_name', 'reset_leaderboard');
            $this->db->set('web_config_value', '');
            $this->db->update('web_config');
            $this->session->set_flashdata('notification', 'Leaderboard set to show all successfully');
            redirect($this->path_to_view_admin . 'appsetting/');
            
        } elseif ($this->input->post('other_submit') == $this->lang->line('text_btn_submit')) {
            if ($this->system->demo_user == 1) {
                $this->session->set_flashdata('error', $this->lang->line('text_err_appsetting'));
                redirect($this->path_to_view_admin . 'appsetting/');
            } else {
//            print_r($_POST);exit;
...
//            $data['one_signal_notification'] = $this->input->post('one_signal_notification');
                ...
                $this->form_validation->set_rules('google_analytics', 'lang:text_google_analytics', 'required', array('required' => $this->lang->line('err_google_analytics_req')));
                ...
                if ($this->input->post('google_analytics') == 'yes') {
                    $this->form_validation->set_rules('google_analytics_code', 'lang:text_google_analytics_code', 'required', array('required' => $this->lang->line('err_google_analytics_code_req')));
                }
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    for ($i = 0; $i < count($settings_arr); $i++) {
                        ...
                        if ($query = $this->db->update('web_config', $settings_data)) {
                            $this->session->set_flashdata('notification', $this->lang->line('text_succ_appsetting'));
                        }
                    }
