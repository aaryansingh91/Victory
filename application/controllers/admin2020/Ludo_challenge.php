    ...
    function upload_result() {
        ...
        if ($this->input->post('submit') == $this->lang->line('text_btn_submit')) {
            ...
            if ($this->system->demo_user == 1) {
                ...
                if( $this->input->post('penlty_user') != "" &&  $this->input->post('penlty') != ""){
                    $user = $this->input->post('penlty_user');
                    $penlty = $this->input->post('penlty');
            ...
            //    $balance = $result['join_money'];
            //    $tot_balance = $balance - $penlty;
             
               if ($result['join_money'] > $penlty) {
                $join_money = $result['join_money'] - $penlty;
                $wallet_balance = $result['wallet_balance'];
            } elseif ($result['join_money'] < $penlty) {
                $join_money = 0;
                $amount1 = $penlty - $result['join_money'];
                $wallet_balance = $result['wallet_balance'] - $amount1;
            } elseif ($result['join_money'] == $penlty) {
                $join_money = 0;
                $wallet_balance = $result['wallet_balance'];
            }
