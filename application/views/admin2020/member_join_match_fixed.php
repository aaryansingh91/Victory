    ...
    <body>
                                ...
                                <table id="example" class="table  table-striped table-bordered">
                                    <thead style="background-color: #343a40;color: #fff;">
                                        <tr>
                                            ...
                                            <th><?php echo $this->lang->line('text_per_kill') . '(' . $this->functions->getPoint() . ')'; ?></th>
                                    ...
                                    <tbody>
                                        <tr role="row" class="odd">
                                            ...
                                            <td><?php echo $this->functions->getPoint() . ' ' . utf8_encode(sprintf('%.' . $this->functions->getCurrencyDecimal($this->system->currency) . 'F', $match['win_prize'])); ?></td>
                                            <td><?php echo $this->functions->getPoint() . ' ' . utf8_encode(sprintf('%.' . $this->functions->getCurrencyDecimal($this->system->currency) . 'F', $match['per_kill'])); ?></td>
                                                ...
                                                if (isset($match_details) && !empty($match_details)) {
                                                    foreach ($match_details as $match_detail) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><input type="text" id="killed<?php echo $cnt; ?>" class="numbers" value="<?php echo isset($match_detail->killed) && $match_detail->killed != '' ? $match_detail->killed : 0; ?>"  onKeyUp="count_total_win(<?php echo $cnt; ?>);" >
        ...
        <script>
            ...
            function count_total_win(i) {
                ...
                var kill = $('#killed' + i).val();
                var win_prize = $('#win_prize' + i).val();
                var bonus = $('#bonus' + i).val();
                $('#win' + i).val(per_kill * kill);
                $('#total_win' + i).val(parseInt(place_point) + (parseInt(per_kill) * parseInt(kill)) + parseInt(win_prize) + parseInt(bonus));
            }
            ...
            function update_match() {
                ...
                if (total_wins != win_prizes) {
                    // Swal.fire({
                    //     icon: 'error',
                    //     title: '<?php // echo $this->lang->line('text_oops'); ?>',
                    //     text: 'Total wins and win prizes must be the same.',
                    // });
                    $.ajax({
                        type: "POST",
                        ...
                        success: function funSuccess(response) {
                            Swal.fire({
                                ...
                                text: '<?php echo $this->lang->line('text_result_succ'); ?>',
                            });
                        },
                        error: function funError(response) {
                            Swal.fire({
                                icon: 'error',
                                title: '<?php echo $this->lang->line('text_error'); ?>',
                                text: '<?php echo $this->lang->line('text_result_succ'); ?>',
                            });
                        }
                    });
