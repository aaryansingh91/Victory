                            ...
                            <div class="card-body">
                                <div class="col-md-12">
                                    <form class="needs-validation" enctype="multipart/form-data" id="validate"
                                        ...
                                        action="<?php if ($Action == $this->lang->line('text_action_add')) { ?><?php echo base_url() . $this->path_to_view_admin ?>game/insert<?php } elseif ($Action == $this->lang->line('text_action_edit')) { ?><?php echo base_url() . $this->path_to_view_admin ?>game/edit<?php } ?>">
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6">
                                                <label
                                                    for="package_name"><?php echo $this->lang->line('text_package_name'); ?><span
                                                        class="required" aria-required="true"> * </span></label>
                                                <input type="text" class="form-control" name="package_name" value="<?php if (isset($package_name))
                                                    echo $package_name;
                                                elseif (isset($game_detail['package_name']))
                                                    echo $game_detail['package_name'] ?>">
                                                    <p><b><?php echo $this->lang->line('text_image_note'); ?> : </b> Package
                                                    name show <a
                                                        href="<?php echo base_url() . $this->system->admin_photo . '/note.png'; ?>"
                                            ...
                                            <div class="form-group col-md-6">
                                                <label
                                                    for="game_logo"><?php echo $this->lang->line('text_logo'); ?><span
                                                        class="required" aria-required="true"> * </span></label><br>
                                                <input id="game_logo" type="file" class="file-input d-block"
                                                    name="game_logo">
                                                <?php echo form_error('game_logo', '<em style="color:red">', '</em>'); ?>
                                                <input type="hidden" id="file-input" name="old_game_logo"
                                                    value="<?php echo (isset($game_detail['game_logo'])) ? $game_detail['game_logo'] : ''; ?>"
                                                    class="form-control-file">
                                                <p><b><?php echo $this->lang->line('text_image_note'); ?> : </b>
                                                    <?php echo "Upload 200x200 size of image for better view in app."; ?>
                                                </p>
                                                <?php if (isset($game_detail['game_logo']) && $game_detail['game_logo'] != '' && file_exists($this->game_logo_image . $game_detail['game_logo'])) { ?>
                                                    <br>
                                                    <img
                                                        src="<?php echo base_url() . $this->game_logo_image . "thumb/100x100_" . $game_detail['game_logo'] ?>">
                                                <?php } ?>
                                            </div>
                                            ...
                                            <div class="form-group col-md-6">
                                                <label for="home_game_id"><?php echo $this->lang->line('text_game'); ?>
                                                    <span class="required" aria-required="true"> * </span></label>
                                                <select class="form-control" name="home_game_id"
                                                    onchange="game_change(this.value);">
                                                    <option value="">Select..</option>
                                                    <?php
                                                    foreach ($home_game as $hgame) {
                                                        ?>
                                                        <option value="<?php echo $hgame->game_id; ?>" <?php
                                                           if (isset($home_game_id) && $home_game_id == $hgame->game_id)
                                                               echo 'selected';
                                                           elseif (isset($game_detail['home_game_id']) && $game_detail['home_game_id'] == $hgame->game_id)
                                                               echo 'selected';
                                                           else
                                                               echo '';
                                                           ?>><?php echo $hgame->game_name; ?></option>
                                                        <?php
                                                    }
                                                    ?>
                                                </select>
                                                <?php echo form_error('home_game_id', '<em style="color:red">', '</em>'); ?>
                                            </div>

                                            <div class="form-group col-12">
                                                <label
                                                    for="game_rules"><?php echo $this->lang->line('text_game_rules'); ?></label>
                                                <textarea type="text" class="form-control ckeditor" id="editor1"
                                                    name="game_rules"><?php
                                                    if (isset($game_rules))
                                                        echo $game_rules;
                                                    elseif (isset($game_detail['game_rules']))
                                        ...
                                        <div class="form-group text-center">
                                            <button class="btn btn-primary" type="submit"
                                                value="<?php echo $this->lang->line('text_btn_submit'); ?>" name="submit"<?php
            ...
            if (game_id != "") {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.game_rules != null) {
                            CKEDITOR.instances.editor1.setData(data.game_rules);
                        }
                    }
                });
            }
