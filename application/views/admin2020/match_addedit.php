                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card bg-light text-dark">
                                ...
                                <?php if ($Action == $this->lang->line('text_action_edit') && isset($only_room_details) && $only_room_details) { ?>   
                                    <!-- ONLY ROOM DETAILS FORM - For users with only_room_details permission -->
                                    <div class="card-body">
                                        <form method="POST" id="validate-form" autocomplete="off" action="<?php echo base_url() . $this->path_to_view_admin ?>matches/edit">
                                            <div class="row">
                                                <div class="col-md-12">
                                ...
                                <?php } else { ?>
                                    <!-- FULL EDIT FORM - For users with full permissions -->
                                        ...
                                        <form class="needs-validation"  enctype="multipart/form-data" id="validate" novalidate="" method="POST" action="<?php if ($Action == $this->lang->line('text_action_add')) { ?><?php echo base_url() . $this->path_to_view_admin ?>matches/insert<?php } elseif ($Action == $this->lang->line('text_action_edit')) { ?><?php echo base_url() . $this->path_to_view_admin ?>matches/edit<?php } ?>">
                                            ...
                                            <div class="row">
                                                <div class="form-group col-md-5">
                                                    ...
                                                    <p><b><?php echo $this->lang->line('text_image_note'); ?> : </b> <?php echo $this->lang->line('text_image_note_150x150'); ?></p>    
                                                    ...
                                                    <?php if (isset($match_detail['match_banner']) && $match_detail['match_banner'] != '' && file_exists($this->match_banner_image . $match_detail['match_banner'])) { ?>
                                                        <br>
                                                        <img src ="<?php echo base_url() . $this->match_banner_image . "thumb/150x150" . $match_detail['match_banner'] ?>" >
                                                    <?php } ?>
                                                </div>
                                                ...
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <select id="id_select2_example" name="image_id" style="width:100%">
                                                        <option value="0" data-img_src=""><?php echo $this->lang->line('text_select'); ?></option>
                                                        <?php
...
const winPrizeInput = document.getElementById('win_prize');
    const entryFeeInput = document.getElementById('entry_fee');
    const winPrizeError = document.getElementById('win_prize_error');

    if (winPrizeInput && entryFeeInput && winPrizeError) {
        winPrizeInput.addEventListener('input', function() {
            const winPrizeValue = parseFloat(winPrizeInput.value);
            const adminProfit = <?php echo $this->system->admin_profit; ?>;
            const entryFeeValue = parseFloat(entryFeeInput.value);
            const maxPrize = entryFeeValue - (entryFeeValue * adminProfit / 100);
            
            if (isNaN(winPrizeValue) || winPrizeValue < 0 || winPrizeValue > maxPrize) {
                winPrizeError.textContent = `Enter a valid value`;
            } else {
                winPrizeError.textContent = "";
            }
        });
    }
