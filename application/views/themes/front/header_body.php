    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            <ul class="navbar-nav ml-auto">
                ...
                <?php $pages = $this->functions->getAllPages();
                    if (!empty($pages) && count($pages) > 0) {                       
                    foreach($pages as $page){
                        $page_slug = $page->page_slug; 
                        $parent_menu = $this->functions->getAllChild($page->page_id);
                    ?>
                        <li class="nav-item <?php if($this->uri->segment('3') == $page_slug ) echo "active";  if(is_array($parent_menu) && count($parent_menu) > 0) echo " dropdown"; ?>">
                        <?php 
